/* const fs = require('fs');
const _ =require('underscore') //구버전
...
// 현재 경로를 workingDir 로 설정하여 하위 폴더로 video, capture 폴더 생성
const workingDir = process.cwd(); // 현재 위치 경로
const videoDIr = path.join(workingDir, 'video'); // 경로 주소만 주는 것이고 실제 생성시키는게 아님
...
function isImgFile(file) {
  ...
  if(imgParsed.ext == '.png' || imgParsed.ext == '.jpg') return true;
}
